...
export function PalladiumSite() {
  ...
  return <main>
    <header><div className="shell nav"><a href="#inicio"><Brand/></a><nav><a href="#pousada">A pousada</a><a href="#acomodacoes">Acomodações</a><a href="#experiencia">Experiência</a><a href="#informacoes">Informações</a><a href="#localizacao">Localização</a></nav><a className="nav__cta" href="#reservar">Reservar <Arrow/></a><button className="menu" onClick={()=>setMenu(!menu)} aria-label="Abrir menu">{menu?"×":"☰"}</button></div>{menu&&<nav className="mobile-nav"><a href="#pousada" onClick={()=>setMenu(false)}>A pousada</a><a href="#acomodacoes" onClick={()=>setMenu(false)}>Acomodações</a><a href="#experiencia" onClick={()=>setMenu(false)}>Experiência</a><a href="#informacoes" onClick={()=>setMenu(false)}>Informações</a><a href="#localizacao" onClick={()=>setMenu(false)}>Localização</a></nav>}</header>
    ...
    <section className="details section" id="informacoes"><div className="shell"><div className="details__head"><div><p className="eyebrow green">Informações da estadia</p><h2>Tudo preparado para a sua chegada.</h2></div><p>Horários e serviços reunidos para você planejar cada detalhe com tranquilidade.</p></div><div className="times"><div className="time-card"><span className="time-icon">↘</span><div><small>Entrada</small><b>14h</b><span>Recepção até 23h</span></div></div><div className="time-card"><span className="time-icon">↗</span><div><small>Saída</small><b>11h</b><span>A partir das 8h</span></div></div><a href={whatsapp} target="_blank" rel="noreferrer"><span><small>Chegada fora do horário?</small><b>Converse com a recepção</b></span><Arrow/></a></div><div className="amenities">{[["wifi","Wi Fi","Gratuito"],["coffee","Café da manhã","Buffet incluso"],["parking","Estacionamento","Privativo e gratuito"],["gym","Academia","Disponível"],["service","Serviço de quarto","Mais comodidade"],["family","Quartos familiares","Para viajar juntos"],["air","Ar condicionado","Em todas as acomodações"],["garden","Jardim e terraço","Para desacelerar"],["meeting","Reuniões","Espaço disponível"]].map(([icon,title,text])=><article key={title}><i><ServiceIcon name={icon}/></i><div><b>{title}</b><small>{text}</small></div></article>)}</div></div></section>
