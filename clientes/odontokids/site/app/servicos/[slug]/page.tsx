...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className={`service-page service-${service.color}`}>
      ...
      <section className="service-hero">
        <div className="shell service-hero-grid">
          <div className="service-hero-copy">
            <Link className="back-link" href="/#servicos"><ArrowLeft /> Voltar ao mapa</Link>
            <p className="eyebrow">Missão {service.number} · {service.kicker}</p>
            <h1>{service.title}</h1>
            <p>{service.summary}</p>
            <div className="service-hero-actions"><a className="primary-btn" href={whatsapp} target="_blank" rel="noreferrer">Agendar avaliação <span className="ico-slot"><ArrowUpRight /></span></a><span className="service-tag">{service.tag}</span></div>
          </div>
      ...
      <section className="service-final"><div className="shell service-final-inner"><div><p className="eyebrow">Prontos para começar?</p><h2>Uma consulta pode mudar a relação da criança com o cuidado.</h2></div><a href={whatsapp} target="_blank" rel="noreferrer">Agendar<br />avaliação <span className="ico-slot"><ArrowUpRight /></span></a></div></section>
      <SiteFooter />
    </main>
  );
}
